...
async function create(req, res) {
	try {
		...
		if (password.trim().length < PASSWORD_MIN_LENGTH) {
			...
			res.status(400).json({ message: `Password too short, must be at least ${PASSWORD_MIN_LENGTH}` })
			return
		}
		...
		console.error(err)
		res.status(500).json({ message: "Service is down, please try again later" });
	}
}

function emailIsValid(email) {
	let formattedEmail = email.trim().toLowerCase()
	if (formattedEmail.length >= EMAIL_MIN_LENGTH && formattedEmail.includes("@")) {
		return true
	}
